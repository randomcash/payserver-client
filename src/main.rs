...
//! the cdylib (`data-target-name="payserver_client"`), so the deployed bundle
//! enters through `payserver_client::init`. Both paths call `mount_app` so the
//! two entry points cannot drift apart.

fn main() {
    payserver_client::mount_app();
}
